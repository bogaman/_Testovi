...
namespace Produkcija
{
...

    {
        ...
        public static void ProveriProxSignStatusAsync()
        {
            const string ProxsignExeName = "proxsign";

            // 1️⃣ Proveri da li postoji proces
            var procesi = Process.GetProcessesByName(ProxsignExeName);
            foreach (var p in procesi)
            {
                try
                {
                    p.Kill();
                    p.WaitForExit();
                    Console.WriteLine("Proxsign zatvoren.");
                }
                catch (Exception ex)
                {
            ...
            // 2️⃣ Ponovo pokreni aplikaciju
            string exePath = @"C:\Program Files (x86)\SETCCE\proXSign\bin\proxsign.exe"; // proveri tačnu putanju
            if (!System.IO.File.Exists(exePath))
            {
                throw new Exception("Proxsign.exe nije pronađen na očekivanoj putanji.");
            }
            ...
            Thread.Sleep(4000); // sačekaj da se aplikacija pokrene


            // 3️⃣ Minimizuj u tray (klikom na X)
            try
            {
                using (var app = FlaUI.Core.Application.Attach(ProxsignExeName))
                using (var automation = new UIA3Automation())
                {
                    var window = app.GetMainWindow(automation);
                    var closeButton = window.FindFirstDescendant(cf => cf.ByAutomationId("Close")) ??
                                      window.FindFirstDescendant(cf => cf.ByName("Close")) ??
                                      window.FindFirstDescendant(cf => cf.ByControlType(FlaUI.Core.Definitions.ControlType.Button));

                    closeButton?.AsButton()?.Invoke();
                    Console.WriteLine("Proxsign minimizovan u system tray.");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Greška pri minimizovanju: {ex.Message}");
            }

            // 4️⃣ Proveri da li je aplikacija u system tray-u
            Thread.Sleep(2000);
            bool isRunning = Process.GetProcessesByName(ProxsignExeName).Any();
            if (isRunning)
            {
                Console.WriteLine("✅ Proxsign je aktivan (u system tray-u).");
            }
            else
            {
                Console.WriteLine("⚠️ Proxsign nije pronađen među aktivnim procesima.");
            }



            /****************************************************
            // Naziv procesa bez .exe ekstenzije
            const string ProcessName = "proxsign";

            // Pronađi sve procese sa tim imenom
            ...
            if (isRunning)
            {
                Console.WriteLine($"{ProcessName}.exe je pokrenut. Nastavljam sa testom...");
                // Ovdje možete dodati Playwright interakcije koje zavise od ProxSign-a
                // Npr. await _page.ClickAsync("#dugme_koje_radi_sa_proxsign");
                // Zatvaranje svake pronađene instance
                foreach (Process proc in processes)
                {
                    try
                    {
                        proc.Kill(); // Brzo zatvaranje procesa
                        proc.WaitForExit(5000); // Sačekaj do 5 sekundi da se proces ugasi
                        Console.WriteLine($"-> Instanca sa ID {proc.Id} zatvorena.");
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"Neuspelo zatvaranje procesa {proc.Id}: {ex.Message}");
                        // Nastavi sa sledećom instancom, ili dodaj ovde logiku za neuspešno zatvaranje.
                    }
                }
            ...
            {
                // Pokreni novi proces
                Process.Start(@"C:\Program Files (x86)\SETCCE\proXSign\bin\proxsign.exe");
                Console.WriteLine("-> Proces uspešno pokrenut.");

                // Moramo sačekati da se prozor aplikacije pojavi.
                var app = FlaUI.Core.Application.AttachOrLaunch(new ProcessStartInfo(@"C:\Program Files (x86)\SETCCE\proXSign\bin\proxsign.exe"));

                // 2. Kreiranje Automation objekta (potrebno za rad sa elementima)
                // NAPOMENA: Ovaj objekat mora biti kreiran *nakon* što aplikacija pokrene prozor.
                // Međutim, za GetMainWindow, često nije direktno potreban ovde, jer se to rešava unutar metode.

                // 3. Čekanje na glavni prozor

                // Dajemo Playwrightu do 20 sekundi da locira glavni prozor aplikacije
                TimeSpan timeout = TimeSpan.FromSeconds(20);

                // Metoda GetMainWindow() je najbolji način. Ona automatski čeka
                // (sa ugrađenim timeoutom) da Windows registruje prozor kao "glavni prozor" procesa.
                var mainWindow = app.GetMainWindow(automation: null, timeout);
                // Parametar automation: null se prosleđuje jer app zna koji engine koristi.

                if (mainWindow == null)
                {
                    // Ako GetMainWindow vrati null nakon timeouta, bacamo izuzetak.
                    app.Close(); // Zatvori proces
                    throw new Exception($"Glavni prozor aplikacije se nije pojavio u roku od {timeout.TotalSeconds} sekundi.");
                }

                Console.WriteLine($"Glavni prozor aplikacije '{app.Name}' uspešno pronađen.");
            ...
            {
                Console.WriteLine($"Neuspelo pokretanje procesa: {ex.Message}");
                // Možeš baciti izuzetak ovde ako je pokretanje kritično
                // throw new ApplicationException($"Nije moguće pokrenuti {fullProcessPath}", ex);
            }

            // Pronađi dugme za zatvaranje (X) - koje bi u teoriji trebalo da minimizuje u tray
            var mainWindow2 = app.GetMainWindow(automation: null, timeout);
            var closeButton = mainWindow2.FindFirstDescendant(cf =>
                cf.ByControlType(ControlType.Button)
                  .And(cf.ByName("Zatvori").Or(cf.ByName("Close"))));
            // Ako je aplikacija lokalizovana, koristi se 'Zatvori'. Ako je na engleskom, 'Close'.

            if (closeButton == null)
            {
                // Alternativno: Pokušaj pronaći dugme po Automation ID-u, ako je poznat.
                Console.WriteLine("Nije pronadjeno dugme 'Zatvori'. Pokušavam da koristim API za minimizaciju.");

                // Ako je dugme zaista sistemsko (Window Close Button),
                // može se koristiti FlaUI za direktno pozivanje funkcije minimizacije
                mainWindow.Minimize();
            }
            else
            {
                Console.WriteLine("Pronadjeno dugme 'Zatvori'. Klikam...");
                closeButton.Click();
            }

            // Sačekaj kratko da se operacija desi
            Thread.Sleep(1000);
*****************************************************/
        ...
        protected static async Task Sertifikat(IPage _page, string SertifikatName)
        {
            ...
            try
            {
                ...
                {
                    System.Windows.MessageBox.Show($"Aplikacija nije pokrenuta. ", "Informacija");

                }
                else
                {
                    System.Windows.MessageBox.Show($"Aplikacija nije pokrenuta. Pokrenuću je...", "Informacija");
                }
        ...
        protected static async Task Sertifikat2(IPage _page, string SertifikatName)
        {
