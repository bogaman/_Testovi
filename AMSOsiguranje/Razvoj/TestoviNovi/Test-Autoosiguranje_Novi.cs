...
namespace Razvoj
{
    ...
    [TestFixture]
    public partial class OsiguranjeVozilaNovi : OsiguranjeNovi
    {

        //private static string baseUrl = Environment.GetEnvironmentVariable("BASE_URL") ?? "ručno";
        ...
        */
        ...
        //LogovanjeTestaNovi.UnesiPocetakTesta();
        //LogovanjeTestaNovi.LogMessage($"Pokrenut je test: {nazivTekucegTesta}");
        ...
        */
        ...
        {
            await Task.Delay(1);
            string putanja = LogovanjeTestaNovi.LogFolder;  // Ovde unesi svoj folder
        ...
        public async Task _TestTestova()
        {
            ...
            try
            {
                ...
                */
                ...
                Console.WriteLine($"******************** logFolder:              {LogovanjeTestaNovi.LogFolder}");
                //Console.WriteLine($"******************** logFolder2:             {logFolder2}");
                Console.WriteLine($"******************** logFajlSumarni:         {LogovanjeTestaNovi.LogFajlSumarni}");
                Console.WriteLine($"******************** logTrace:               {LogovanjeTestaNovi.LogFajlTrace}");
                Console.WriteLine($"******************** logFajlOpsti:           {LogovanjeTestaNovi.LogFajlOpsti}");
                Console.WriteLine($"******************** putanjaDoBazeIzvestaja: {LogovanjeTestaNovi.PutanjaDoBazeIzvestaja}");
            ...
            catch (Exception ex)
            {
                ...
                LogovanjeTestaNovi.LogException(ex, kontekst);
                LogovanjeTestaNovi.LogTestResult(nazivTekucegTesta, false);
            ...
            try
            {
                using (OleDbConnection konekcija = new OleDbConnection(LogovanjeTestaNovi.ConnectionStringLogovi))
                {
                    konekcija.Open();
                    ...
                    using (OleDbCommand komanda = new OleDbCommand(sqlUpit, konekcija))
                    {
                        ...
                        LogovanjeTestaNovi.LogMessage($"✅ Uspešno ubačeno {brojRedova} redova u bazu.", false);
                    }
                }
            ...
            {
                LogovanjeTestaNovi.LogError($"❌ Greška prilikom upisa u bazu: {ex.Message}");
                throw; // Ponovo baca grešku, da NUnit zna da je test neuspešan
            }

            Console.WriteLine($"{PocetnaStrana} - {PocetnaStranaWS}");
        ...
        public async Task _TestTestova_radni_1(string baseUrl)
        {
            await _page!.PauseAsync();
        ...
        public async Task _TestTestova_radni_2(string baseUrl)
        {
            await _page!.PauseAsync();
        ...
        public async Task _ProveraDashboard()
        {
            ...
            await ProveriVidljivostKontrole(_page, "//i[@class='ico-user-secret']", "ikona korisnika");
            ...
            await ProveriPostojanjeKontrole(_page, "//e-button[@id='btnNovaVest']", "dugme Nova vest");

            // Proveri da li stranica sadrži grid Vesti
            await ProveraPostojiGrid(_page, "//e-grid[@id='grid_vesti']", "grid Vesti");

            // Provera postojanja barem jedne vesti
            await ProveriPostojanjeKontrole(_page, "//div[starts-with(@class, 'vestiBox id_')]", "barem jedna vest");

            // Provera postojanja brojaca
            await ProveriPostojanjeKontrole(_page, "//button[@class='left primary flat btn-page-num num-1 flex-center-center']", "levi brojač");
            await ProveriPostojanjeKontrole(_page, "//button[@class='left primary flat flex-center-center']", "znak -");
            await ProveriPostojanjeKontrole(_page, "//div[@class='control']//input[@class='input']", "aktuelna strana vesti");
            await ProveriPostojanjeKontrole(_page, "//button[@class='left primary flat right flex-center-center']", "znak +");
            await ProveriPostojanjeKontrole(_page, "//button[@class='left primary flat btn-page-num num-max flex-center-center']", "desni brojač");
            ...
            *************************************************/
            ...
            await ProveraPostojiGrid(_page, "//e-grid[@id='grid_dokumenti']", "grid Dokumenti - polise putnog osiguranja");
        ...
        public async Task AO_1_SE_PregledPretragaObrazaca()
        {
            ...
            try
            {
                ...
                */
                ...
                {
                    LogovanjeTestaNovi.LogException(ex, $"Greška prilikom pokušaja klika na ćeliju sa vrednošću '{serijskiBrojObrasca}'.");
                }
                ...
                try
                {
                    ...
                    foreach (string value in expectedValues)
                    {
                        ...
                        {
                            allFound = false;
                            LogovanjeTestaNovi.LogError($"Nije pronađen tekst boks sa vrednošću: '{value}'");
                        }
                        else
                        {
                            LogovanjeTestaNovi.LogMessage($"Uspešno pronađen tekst boks sa vrednošću: '{value}'");
                        }
                    }

                    if (allFound)
                    {
                        LogovanjeTestaNovi.LogMessage("Svi očekivani tekst boksovi su pronađeni sa tačnim vrednostima.");
                    }
                    else
                    {
                        LogovanjeTestaNovi.LogError("Neki od očekivanih tekst boksova nisu pronađeni.");
                    }
                }
                catch (Exception ex)
                {
                    LogovanjeTestaNovi.LogException(ex, "Greška prilikom provere tekst boksova sa vrednostima prom1–prom4.");
                }
                ...
                //Assert.Pass();
                LogovanjeTestaNovi.LogMessage($"✅ Uspešan test {nazivTekucegTesta} ");
            }
            catch (Exception ex)
            {
                LogovanjeTestaNovi.LogError($"❌ Neuspešan test {nazivTekucegTesta} - {ex.Message}");
                throw;
            }
        }
        ...
        public async Task AO_2_SE_PregledPretragaDokumenata()
        {
            ...
            try
            {
                ...
                */
                ...
                {
                    LogovanjeTestaNovi.LogException(ex, $"Greška prilikom pokušaja klika na ćeliju sa vrednošću '{oznakaDokumenta}'.");
                }
                //await _page.Locator($"//div[@class='podaci']//div[contains(@class, 'column') and normalize-space(text())='{serijskiBrojObrasca}']").ClickAsync();
                //await _page.PauseAsync();
                ...
                try
                {
                    ...
                    foreach (string value in expectedValues)
                    {
                        ...
                        {
                            allFound = false;
                            LogovanjeTestaNovi.LogError($"Nije pronađen element sa vrednošću: '{value}'");
                        }
                        else
                        {
                            LogovanjeTestaNovi.LogMessage($"Uspešno pronađen element sa vrednošću: '{value}'");
                        }
                    }

                    if (allFound)
                    {
                        LogovanjeTestaNovi.LogMessage("Svi očekivani elementi su pronađeni sa tačnim vrednostima.");
                    }
                    else
                    {
                        LogovanjeTestaNovi.LogError("Neki od očekivanih elemenata nisu pronađeni.");
                    }
                ...
                {
                    LogovanjeTestaNovi.LogException(ex, $"Greška prilikom provere elemenata sa vrednostima: {expectedValues}.");
                }
        ...
        public async Task AO_3_SE_UlazPrenosObrazaca()
        {
            ...
            try
            {
                ...
                ******************************************/
                ...
                LogovanjeTestaNovi.LogMessage($"✅ Poslednji mejl -> ID: {PrethodniZapisMejla.PoslednjiID}, IDMail: {PrethodniZapisMejla.PoslednjiIDMail}, Status: {PrethodniZapisMejla.Status}, Opis: {PrethodniZapisMejla.Opis}, Datum: {PrethodniZapisMejla.Datum}, Subject: {PrethodniZapisMejla.Subject}", false);
                ...
                ocekivaniTekst = $"Dokument \"Ulaz u centralni magacin\" je vraćen u izradu.\n" +
                                 $"Dokument možete pogledati klikom na link: {oznakaDokumenta}";
                LogovanjeTestaNovi.LogMessage($"❌ Proverava se da li je vest obrisana nakon brisanja dokumenta {oznakaDokumenta}.", false);
                //await ProveraVestPostoji(_page, ocekivaniTekst);
                ...
                ********************************************************/
                ...
                await ProveriURL(_page, PocetnaStrana, $"/Stroga-Evidencija/1/Autoodgovornost/Dokument/1/{PoslednjiDokument + 1}");
                LogovanjeTestaNovi.LogMessage($"✅ Otvaranje dokumenta iz grida: {PoslednjiDokument + 1} - {oznakaDokumenta}.", false);
                PrethodniZapisMejla = await ProcitajPoslednjiZapisMejla();
                LogovanjeTestaNovi.LogMessage($"✅ Poslednji mejl -> ID: {PrethodniZapisMejla.PoslednjiID}, IDMail: {PrethodniZapisMejla.PoslednjiIDMail}, Status: {PrethodniZapisMejla.Status}, Opis: {PrethodniZapisMejla.Opis}, Datum: {PrethodniZapisMejla.Datum}, Subject: {PrethodniZapisMejla.Subject}", false);
                ...
                *****************************************/
                ...
                LogovanjeTestaNovi.LogMessage($"❌ Proverava se da li je vest arhivirana za agenta nakon vraćanja u izradu: {oznakaDokumenta}.", false);
        ...
        public async Task AO_4_Polisa(string _rb, string _tipPolise, string _premijskaGrupa, string _popusti, string _tipUgovaraca, string _tipLica1, string _tipLica2, string _brojDana, string _tegljac, string _podgrupa, string _zapremina, string _snaga, string _nosivost, string _brojMesta, string _oslobodjenPoreza, string _mb1, string _mb2, string _pib1, string _pib2, string _platilac1, string _platilac2)
        {
            ...
            {
                brojPokusaja++;
                LogovanjeTestaNovi.LogMessage($"Pokušaj #{brojPokusaja}: Klik na dugme 'Izračunaj'");
                ...
                try
                {
                    ...
                    if (neuspeh)
                    {
                        LogovanjeTestaNovi.LogMessage("Pojavila se poruka o neuspešnom kalkulisanju.");
                        ...
                        if (await dugmeZatvori.IsVisibleAsync())
                        {
                            ...
                            LogovanjeTestaNovi.LogMessage("Poruka zatvorena.");
                        }
                    ...
                    {
                        LogovanjeTestaNovi.LogMessage("Kalkulisanje je uspešno.");
                        break;
                    }
                }
                catch (Exception ex)
                {
                    LogovanjeTestaNovi.LogException(ex, $"Greška tokom pokušaja #{brojPokusaja}");
                }
            }

            if (neuspeh)
            {
                LogovanjeTestaNovi.LogError("Kalkulisanje neuspešno nakon 5 pokušaja.");
        ...
        */
        ...
        [Test]
        public async Task AO_5_ZahtevZaIzmenom()
        {
            await _page!.PauseAsync();
            try
            {
                ...
                await _page.GetByText($"Imate novi zahtev za izmenomDokument možete pogledati klikom na link: {dok}").ClickAsync();
            ...
            {
                LogovanjeTestaNovi.LogException(ex, "Greška u testu");
                LogovanjeTestaNovi.LogError($"❌ Neuspešan test {nazivTekucegTesta} - {ex.Message}");
            }
        ...
        [Test]
        public async Task AO_6_RazduznaLista()
        {
            try
            {
                await _page!.PauseAsync();
...
if(brojUgovora != "")
        ...
        public async Task _TestSvegaISvacega()
        {
            ...
            await ProveriKontrolu(_page, "//e-select[@id='selPartner']//div[@class='multiselect-dropdown input']");
            await ProveriKontrolu(_page, "//e-select[@id='selTarife']//div[@class='multiselect-dropdown input']");
...
");
        ...
        */
        ...
        [Test]
        public async Task GenerisiBrojSasije()
        {
        ...
        {
            //await NovaPolisa(_page, "Nova polisa OP");
            await NovaPolisa(_page!, "Novi zeleni karton");
            //await NovaPolisa(_page, "Nova polisa - lom");
            //await ProveriURL(_page, Variables.PocetnaStrana, "/Osiguranje-vozila/7/Lom-stakla-auto-nezgoda/Dokument/0");
            await _page!.PauseAsync();
